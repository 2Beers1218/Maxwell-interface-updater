...
template <int dim>
SurfaceCommunicator<dim>::SurfaceCommunicator(unsigned int n_domains_, std::string filename, unsigned int n_face_q_points, unsigned int n_rows) {
  n_domains = n_domains_;
  ...
  for(unsigned int i = 0; i < n_domains * n_domains; i++){
    ...
    std::vector< std::vector< Tensor<1, dim, std::complex<double>>>> g_value(n_rows);

    // --- Einlesen ---
    // lese die csv als matrix ein, z.B.: als std::vector<std::vector<std::complex<double>>> csv_input;
    for( unsigned int i = 0; i < n_domains; i++ ) {
        for( unsigned int j = 0; j < n_domains; j++ ) {

            int diff_i_j = i-j;
            if(std::abs(diff_i_j) == 1) {
                std::string filename_i_j = filename + std::to_string(i) + "_" + std::to_string(j) + "_nn" + ".csv";
                //std::cout << filename_i_j << std::endl;
                std::ifstream file(filename_i_j);
                std::vector<std::vector<std::complex<double>>> csv_input; // TODO
                ...
                for ( unsigned int row = 0; row < n_rows; row++ ) {
                    ...
                    for ( unsigned int q_point = 0; q_point < n_face_q_points; q_point++ ) {
                        // Aufbau von jedem Elemente: (Re, Im) (Re, Im) (fuer dim = 2)
                        // hier: (a, b) (c, d)
                        // TODO: Ich bin mir nicht sicher ob die Syntax so funktioniert
                        std::complex<double> ab = csv_input[row][2 * q_point];
                        std::complex<double> cd = csv_input[row][2 * q_point + 1];
                        update_value[q_point] = Tensor<1, dim, std::complex<double>>({ab, cd});
